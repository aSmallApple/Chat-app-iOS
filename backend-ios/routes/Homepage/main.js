...
module.exports = function(app, objJson, isEmailValid){
    ...
    app.post("/login", function(req, res){
        if(!req.body.Email || !req.body.Password){
            res.json({result:0, message:"Wrong parameters", token:""});
        }else{
            var email = req.body.Email.trim().toLowerCase();
            var password = req.body.Password;
            ...
            .then((user)=>{
                if(user!=null){
                    bcrypt.compare(password, user.Password, function(err, resB) {
                        if(err){    
                            res.json({result:0, message:"Password checking is invalid", token:""});
                        }else{
                            console.log(resB);
                            if(resB===true){
                                ...
                                jwt.sign({
                                    ...
                                    if(errT){
                                        res.json({result:0, message:"Token is invalid", token:""});
                                    }else{
                                        ...
                                        newToken.save().then(()=>{
                                            ...
                                            res.json({result:0, message:"Token is saved failed.", token:""});
                                        });
                                ...
                                res.json({result:0, message:"Wrong password.", token:""});
                            }
                        }   
                    });
                    ...
                    res.json({result:0, message:"Email is not registered.", token:""});
                }
            })
            .catch((e)=>{
                res.json({result:0, message:"Found user is invalid", token:""});
            });
        }
    });
                ...
                if(t==null){
                    ...
                    jwt.verify(token, objJson.secretKey, function(err, decoded) {
                        if (err) {
                            ...
                            res.json({result:1, message:"Token is okay",
